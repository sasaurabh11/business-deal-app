...
const getDealsStatistics = async (req, res) => {
  try {
    ...
    const [pendingDeals, completedDeals] = await Promise.all([
      dealModel.countDocuments({ status: "Pending" }),
      dealModel.countDocuments({ status: "Completed" })
    ]);

    res.status(200).json({ success: true, data: { pendingDeals, completedDeals } });
  } catch (error) {
...
const getUserEngagement = async (req, res) => {
  try {
    ...
    const activeUsers = await userModel.countDocuments({ lastLogin: { $gte: oneWeekAgo } });
